...
export default async function initializeTelegramBot(manager) {
    // ── CONFIG ──────────────────────────────────────────────────────────────────
    const ALLOWED_GROUP_ID = Number(process.env.TG_GROUP_ID) || -1003914452690;
    const GROUP_INVITE_LINK =
        process.env.TG_GROUP_LINK || "https://t.me/cutie_mdbot";
    const WA_CHANNEL_LINK =
        process.env.WA_CHANNEL_LINK ||
        "https://whatsapp.com/channel/0029Vb82I0Z3AzNO81cHHz1k";
    const PAIR_COOLDOWN_MS = 30_000; // 30s between pair attempts per user

    const BOT_TOKEN =
        process.env.BOT_TOKEN_TELEGRAM ||
        process.env.BOT_TOKEN ||
        "8103000532:AAERWnemil2CT1NGe6n98tALBB5LX7rTHRg";
...
- ɴᴜᴍʙᴇʀ - ${esc(rawArg)}</b>`,
        ...
        try {
            ...
            rawCode = await sock.requestPairingCode(digits, 'CUTIEBOT');
        } catch (err) {
